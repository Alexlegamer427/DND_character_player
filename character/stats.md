# Statistiques du personnage

`` Contient les statistiques et affinitées avec les saving throw et proficiencies du personnage ``

## Stats

|Stats Type|Amount|Bonus|
|---|---|---|
|Constitution|14|+2|
|Strength|8|-1|
|Dexterity|(16) 14+2|+3|
|Constitution|8|-1|
|Constitution|16|+3|
|Constitution|(16) 15+1|+3|

## Proficiencies & Skill checks

|Proficiency|Mod|Obtained|Bonus|
|---|---|---|---|
|Trading|Extra|X|+???|
|Acrobatics|Dexterity|X|(+5)+2p+3s|
|Animal Handling|Wisdom||-1|
|Arcana|Intelligence|X|(+5)+2p+3s|
|Athletics|Strength|||
|Deception|Charisma|||
|History|Intelligence|||
|Insight|Wisdom||-1|
|Intimidation|Charisma||+3|
|Investigation|Intelligence|X|(+5)+2p+3s|
|Medicine|Wisdom||-1|
|Nature|Intelligence||+3|
|Perception|Wisdom|-1||
|Performance|Charisma||+3|
|Persuasion|Charisma|X|(+5)+2p+3s|
|Religion|Intelligence|||
|Sleight of Hand|Dexterity|||
|Survival|Wisdom|||

*Legend :*

- s = Stats bonus
- p = Proficiency bonus

## Background

`` Basiquement un hunter ``

- [Background Description](character.md)
- Background Feat : [Scout](https://dnd5e.wikidot.com/fighter:scout-ua)

## Feats